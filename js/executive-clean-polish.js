function ensureCleanPdfStyles() {
  ...
  style.textContent = `
    ...
    .pdf-cover__frame::before,
    .pdf-cover__frame::after,
    .pdf-cover__grid {
    ...
    .pdf-cover__pillars div {
      ...
      border-top: 4px solid var(--pdf-accent, #fa0115) !important;
    }

    .pdf-cover__pillars span {
      color: var(--pdf-accent, #fa0115) !important;
      font-size: 11px !important;
      font-weight: 950 !important;
    }

    .pdf-cover__pillars strong {
      margin-top: 11px !important;
      font-size: 15px !important;
    }

    .pdf-cover__pillars small {
    ...
    .pdf-cover__logos {
      width: 100% !important;
      display: grid !important;
      grid-template-rows: 1fr 42px 1fr !important;
      gap: 18px !important;
      align-self: stretch !important;
    }

    .pdf-cover__logo-card {
    ...
    .pdf-summary__objective {
      ...
      border-left: 6px solid var(--pdf-accent, #fa0115) !important;
    }

    .pdf-summary__objective p {
    ...
    .pdf-summary__facts {
      grid-template-columns: 1.25fr .85fr .85fr .85fr !important;
      gap: 12px !important;
      margin-top: 18px !important;
    }
    ...
    .pdf-edney__photo::after {
      content: '' !important;
      position: absolute !important;
      inset: 15px !important;
      border: 3px solid var(--pdf-accent, #fa0115) !important;
      border-radius: 8px !important;
    ...
    .pdf-edney__experience {
      ...
      border-left: 5px solid var(--pdf-accent, #fa0115) !important;
    }
    ...
    .pdf-edney__content .pdf-page__title {
      font-size: 36px !important;
    }

    .pdf-edney__strategy-strip {
      grid-template-columns: repeat(3, minmax(0, 1fr)) !important;
      gap: 12px !important;
    ...
    .pdf-scope-investment .pdf-page__title {
      max-width: 780px !important;
      font-size: 37px !important;
    }

    .pdf-scope-investment .pdf-page__subtitle {
      max-width: 810px !important;
      font-size: 13px !important;
      line-height: 1.52 !important;
    }

    .pdf-scope-investment__layout {
    ...
    .pdf-scope-investment__section-head {
      padding-bottom: 13px !important;
      border-bottom: 1px solid rgba(255,255,255,.13) !important;
    }

    .pdf-scope-investment__groups {
    ...
    .pdf-scope-investment__group li {
      grid-template-columns: 8px minmax(0, 1fr) !important;
      gap: 7px !important;
    ...
    .pdf-scope-investment__commercial {
      ...
      background: rgba(250,1,21,.09) !important;
    }
    ...
    .pdf-scope-investment__value strong {
    ...
    .pdf-closing__headline .pdf-page__title {
    ...
    .pdf-closing__contact-card {
      padding: 24px !important;
      border-top: 5px solid var(--pdf-accent, #fa0115) !important;
      background: rgba(250,1,21,.10) !important;
    }
  `;
...
function updateCleanCopy(root = document) {
  root.querySelectorAll('.pdf-cover__title').forEach((title) => {
    title.innerHTML = 'Estratégia clara.<br>Execução forte.<br>Resultado visível.';
  });
  ...
  root.querySelectorAll('.pdf-cover__pillars div').forEach((pillar, index) => {
    ...
    if (index === 1) {
      if (strong) strong.textContent = 'Ritmo';
      if (small) small.textContent = 'Plano de execução com operação e acompanhamento.';
    }
    if (index === 2) {
      if (strong) strong.textContent = 'Conversão';
      if (small) small.textContent = 'Foco em avanço comercial e vendas.';
    }
  });

  root.querySelectorAll('.pdf-edney').forEach((pageBody) => {
    const page = pageBody.closest('.pdf-page');
    if (page) page.dataset.section = 'Edney Ulisses';
  });
}
...
function renumberCleanPages(root = document) {
  ...
  document.querySelectorAll('.proposal-preview__stack').forEach((stack) => {
    ...
    stack.querySelectorAll('.proposal-preview__sheet').forEach((sheet, index) => {
      ...
      if (first) first.textContent = `Página ${String(index + 1).padStart(2, '0')}`;
      const page = sheet.querySelector('.pdf-page');
      const last = sheet.querySelector('.proposal-preview__sheet-heading span:last-child');
      if (last && page) last.textContent = page.dataset.section || 'Proposta';
    });
  });
  ...
  pageSets.forEach((pages) => {
    ...
    pages.forEach((page, index) => {
      ...
      const coverTotal = page.querySelector('.pdf-cover__meta div:last-child strong');
      if (coverTotal) coverTotal.textContent = totalNumber;
    });
  });
}
...
runCleanPolish();
const cleanObserver = new MutationObserver(() => runCleanPolish());
